...
After resolving, run whatever checks you think are warranted — `cargo clippy -- -D warnings` is fast and catches most resolution mistakes; `cargo test` if you want stronger confidence. You decide. If something is broken, fix what you can; if you can't, push what you have and flag it clearly in the comment.

# COMMIT

Stage everything and finish the merge with a single commit. Use a past-tense descriptive message, e.g. `Merged origin/{{BASE_REF}} into {{BRANCH}} and resolved conflicts`. The wrapper will push whatever you commit.
