...
For anything that looks suspicious — fragile logic, unchecked assumptions, potential panics (`.unwrap()` on fallible paths), missing error propagation, unsafe blocks without justification — write a test that exercises it. Try to actually break it. If you can break it, fix it.
...
- Empty vectors, empty strings, zero, negative numbers
- `None` values, missing fields, unexpected enum variants
- Concurrent access, race conditions (especially around SQLite connections from the r2d2 pool)
- Off-by-one errors in loops or slice operations
- Integer overflow, index out of bounds
- Invalid UTF-8, malformed tree-sitter input
...
- Prefer `?` operator over verbose match/unwrap chains
...
1. Run `cargo fmt -- --check`, `cargo clippy -- -D warnings`, and `cargo test` — confirm the current state passes.
2. Make improvements + write any new edge-case tests. Stage and commit them as a **single squashed commit** on this branch with a past-tense descriptive message. Example: `Hardened edge cases and improved error handling in symbol resolver`
3. Run `cargo fmt -- --check`, `cargo clippy -- -D warnings`, and `cargo test` again. If any fail, fix before continuing — do not leave the branch broken.
