...
Read `CLAUDE.md` for build instructions. Read `CONTEXT.md` and any relevant
ADRs under `docs/adr/` before starting. Explore the repo and fill your
context with the parts relevant to this sub-issue — especially test files
that touch the area you'll change.
...
Before committing, run `cargo fmt`, `cargo clippy -- -D warnings`, and `cargo test`.

# COMMIT

Make one or more git commits on `{{BRANCH}}`. Use past-tense descriptive
messages (no conventional-commit prefixes).
