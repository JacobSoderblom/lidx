...
2. Read `CONTEXT.md` and skim `docs/adr/` for any decisions that bear on
   ...
   project's vocabulary (Symbol Graph, Edge Kind, Bridge Edge, etc.).
...
vertical cut through every layer (schema/migrations -> core logic -> RPC handler -> tests), NOT a
...
- Each slice must stand on its own in a single agent session. A reasonable
  ...
  clippy/test. Don't draft slices that are unrealistic for one session.
  ...
  "slices": [
    {
      ...
      "acceptanceCriteria": [
        ...
        "cargo test passes with new tests covering the behavior"
      ]
    }
  ]
}
...
- `whatToBuild` — prose, not a list. Avoid specific file paths or code
  snippets. Exception: a prototype-derived snippet (state machine, schema,
  type shape) may be inlined when prose can't encode the decision as
  precisely.
